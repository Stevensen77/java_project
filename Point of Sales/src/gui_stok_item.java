...
public class gui_stok_item extends JFrame implements ActionListener  {
	
	//           Pendeklarasian Member Variabel
	
	private JPanel panel1, panel2, panel3,panel4,panel5,panel6,panel7,panel8,panel9,panel10;
	private JButton button1,button2,button3,button4,btn_cari,btn_stok,btn_logout;
	...
	static JFrame frame,frame_stok_item,frame_menu_item,frame_menu_admin,framelogin,frame_laporan;
	private static String nama_login;
	 JTable table;
	public static  JTable tampil,tampil_data_dicari,table_cari;
	static String[] data_dicari;
	static String a,b,c,d,e,kode_huruf,str_kode_angka;
	static boolean stok;
	private static int kuantitas_awal,kuantitas_stok,kuantitas_akhir;
	private static JDialog dg;  

	
	//String columnNames[] = {"Nama barang","Kuantitas"};
	//String data[]= { " ", " ", " ", " ", " " };
	static DefaultTableModel model = new DefaultTableModel(new String[]{"Nama barang","Kode Barang","Kuantitas"}, 0);
	...
	public static void cari_item() throws FileNotFoundException, SQLException,ArrayIndexOutOfBoundsException,NullPointerException
	{
		...
		model.setRowCount(0);
		data_dicari= entity_stok_item.cari_item(kode_huruf,str_kode_angka);
		
		String kode_ketemu=data_dicari[1]+data_dicari[2];
		
		kuantitas_awal = Integer.parseInt(data_dicari[3]);
		
		System.out.println(kuantitas_awal+"awall");

		 model.addRow(new Object[]{data_dicari[0], kode_ketemu,data_dicari[3]});
	...
	public static void stok_item() throws FileNotFoundException, SQLException,ArrayIndexOutOfBoundsException,NullPointerException
	{
		try {
		...
		Object kuantitas_barang= GetData(model, 0, 2);
		
		String str_kode_barang=String.valueOf(kode_barang);
		
		kuantitas_stok=Integer.valueOf((String) kuantitas_barang);
		
		kuantitas_akhir=kuantitas_awal+kuantitas_stok;
		
		boolean cek_input=ctrl_stok_item.cek_input(kode_barang, kuantitas_barang);
		
		if(cek_input==true)
		{
			  kode_huruf = str_kode_barang.substring(0,3);
				//kode_angka=Integer.parseInt(kode);
				str_kode_angka = str_kode_barang.substring(3,5);
		
		stok= entity_stok_item.stok_item(nama_barang,kode_huruf,str_kode_angka,kuantitas_akhir,waktu,nama_login);
		
		
			if(cek_input&&stok==true)
			{
				JOptionPane.showMessageDialog(null, "Stok sukses");
				
			}
			else 
			{
				JOptionPane.showMessageDialog(null, "Stok Item gagal");
		...
		{
			JOptionPane.showMessageDialog(null, "Kesalahan input data,Stok gagal");
		...
		System.out.println("Nilai field dari 1 column dan 1 row :" +kuantitas_barang+waktu+nama_login);	
	...
	gui_stok_item() throws FileNotFoundException, SQLException{		
		...
		lblJudul = new JLabel("MENU STOK ITEM");
		...
		TitledBorder keterangan3 = BorderFactory.createTitledBorder(blackline,"SETTING");
		...
		try {
	        ...
	        {  
	            public void actionPerformed( ActionEvent e )  
	            {  
	               gui_stok_item.dg.setVisible(false);  
	            }  
	        });    
			...
			gui_stok_item.dg.setVisible(true);  
		}
		...
		btn_stok = new JButton("Stok Barang");
		btn_stok.setPreferredSize(new Dimension(150, 50));
